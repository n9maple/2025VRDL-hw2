...
def main():
    ...
    parser.add_argument("--box_pred_path", type=str, default="save_result/pred.json", help="Path to the box prediction json file")
    parser.add_argument("--save_path", type=str, default="save_result/pred.csv", help="Path to the saved result")
    parser.add_argument("--score_thre", type=float, default=0.8, help="The score threshold to pick the box")
...
def num_pred(box_pred_path, save_path, score_thre):
    ...
    with Progress(
        ...
        for box in box_data:
            if box['score']>=score_thre:
                x_center = box['bbox'][0]+box['bbox'][2]/2 # find the x-center of the box
                box_dict.setdefault(box['image_id'], []).append((box['category_id']-1, x_center))
    ...
    with Progress(
        ...
        for image_id in box_dict:
            sorted_digit = sorted(box_dict[image_id], key=lambda x: x[1]) # sort by x_center
